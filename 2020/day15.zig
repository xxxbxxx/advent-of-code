...
fn PackedBoolsArray(comptime size: usize) type {
    return struct {
        ...
        fn setAll(self: *@This(), val: bool) void {
            @memset(@ptrCast([*]u8, &self.words), if (val) @as(u8, 0xFF) else @as(u8, 0x00), @sizeOf(u64) * self.words.len);
        }
        fn get(self: *@This(), index: usize) bool {
            const word = self.words[index / 64];
            const bit = @intCast(u6, index % 64);
...
pub fn run(input_text: []const u8, allocator: std.mem.Allocator) ![2][]const u8 {
    ...
    const ans1 = ans: {
        ...
        for ([_]u16{ 0, 3, 1, 6, 7, 5 }) |n, i| {
            turn = @intCast(u16, i + 1);
            if (i > 0) {
    ...
    const ans2 = ans: {
        ...
        std.mem.set(u32, last_turn_of, 0);
        ...
        for ([_]u32{ 0, 3, 1, 6, 7, 5 }) |n, i| {
            turn = @intCast(u32, i + 1);
            if (i > 0) {
                last_turn_of[last_spoken] = (turn - 1);
